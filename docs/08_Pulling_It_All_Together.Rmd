...
- Read in the tracts ACS joined data (created in Lesson 7)
- Read in a CSV file of grocery store data into a standard R data.frame (it lives at `'notebook_data/other/ca_grocery_stores_2019_wgs84.csv`)
...
- Datasets used
    - 'outdata/tracts_acs_sdf_ac.json'
...
tracts_acs_ac = st_read(here( .. ))
```

```{r,eval=FALSE}
# read our grocery-data CSV into a plain data.frame

grocery_pts_df = read.csv(here( ... ))
```
...
#-----------------------
# read in tracts acs data
tracts_acs_ac = st_read(here('outdata/tracts_acs_sdf_ac.json'))
# read in the grocery-store data
grocery_pts_df = read.csv(here('notebook_data/other/ca_grocery_stores_2019_wgs84.csv'))
