...
2. Start RStudio and open a **new script**, or **./docs/09_Raster_Data.Rmd**
...
```{r, eval=F}
our_packages<- c("ggplot2", "sf", "tmap", "raster", "here")
...
library(here)   # reproducible directory paths
...
SFtracts <- st_read(here("./notebook_data/part_3"), "sftracts_wpop")
```
...
SFhomes <- read.csv(here('./notebook_data/part_3/sf_properties.csv'), 
...
DEM = raster(here('./notebook_data/part_3/san_francisco-e.DEM'))
```
...
```{r}
#read in nlcd data
nlcd = raster(here('./notebook_data/part_3/nlcd2011_sf.tif'))
...
[Here is a link](https://www.mrlc.gov/data/legends/national-land-cover-database-2011-nlcd2011-legend) to the NLCD legend.
...
karl_files <- unique(gsub('.aux.xml', '', list.files(here("notebook_data/part_3/CalMnYr"))))
...
```{r}
karl <- stack(paste0(here('./notebook_data/part_3/CalMnYr/'), karl_files))
